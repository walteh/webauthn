...
func (aar *AuthenticatorAssertionResponse) Verify(storedChallenge string, relyingPartyID string, previousCounter uint32, publicKey []byte) (uint32, error) {
	...
	if x == nil {
		return 0, ErrParsingData.WithDetails("Failed to parse the public key")
	}
	...
	if !valid {
		return 0, ErrAssertionSignature.WithDetails("Error validating the assertion signature.\n")
	}
	...
	if !bytes.Equal(a.AuthenticatorData.RPIDHash[:], rpIDHash[:]) {
		return 0, ErrVerification.WithDetails(fmt.Sprintf("RP Hash mismatch. Expected %x and Received %x\n", a.AuthenticatorData.RPIDHash, rpIDHash))
	}
	...
	if a.AuthenticatorData.Counter <= previousCounter {
		return 0, ErrVerification.WithDetails(fmt.Sprintf("Counter was not not greater than previous  %d\n", a.AuthenticatorData.Counter))
	}
	...
	if storedChallenge != aar.ClientDataJSON.Challenge {
		err := ErrChallengeMismatch.WithDetails("Error validating challenge")
