...
import (
	...
	mylambda "github.com/walteh/webauthn/pkg/lambda"
...
func TestLambdaHandlerSignatures(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("testCase[%d] %s", i, testCase.name), func(t *testing.T) {
			// lambdaHandler := InstrumentHandlerFunc(context.Background(), testCase.handler, otel.GetTracerProvider())
...
func TestHandlerInvokes(t *testing.T) {
	...
	testCases := []struct {
		...
		// },
		{
			...
			input:    "Lambda",
			expected: expected{`"Hello Lambda!"`, nil},
			handler: func(ctx context.Context, name any) (any, error) {
		...
		},
		{
			...
			handler: func(ctx context.Context, event interface{}) (interface{}, error) {
				return event.(struct{ Custom int }).Custom, nil
			},
		},
		// {
	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("lambdaHandlerTestCase[%d] %s", i, testCase.name), func(t *testing.T) {
			...
			// }
			// response, err := lambdaHandler(mockContext, testCase.input)
	...
	// 		handler := WrapHandler[any, any](lambda.NewHandler(testCase.handler))
...
func BenchmarkInstrumentHandler(b *testing.B) {
	...
	// customerHandler := func(ctx context.Context, payload int) (int, error) {
	// 	return 0, nil
	// }
	// wrapped := InstrumentHandlerFunc(context.Background(), customerHandler, otel.GetTracerProvider())
...
func BenchmarkWrapHandler(b *testing.B) {
	...
	// wrapped := WrapHandler[any, any](emptyHandler{})
...
func Test_extractXRayContextIfExists(t *testing.T) {
	type args struct {
		mc propagation.TextMapCarrier
		ev interface{}
	}
	tests := []struct {
		name string
	...
	}{
		{
			...
			args: args{
				...
				ev: &mylambda.SQSEventViaSNS[int]{
					Records: []mylambda.SQSEntityViaSNS[int]{
						{
							Attributes: map[string]string{
								"AWSTraceHeader": "Root=1-63f13f3b-4aed39aafbf3701390990cf1;Parent=ea243272ffa5c50b;Sampled=1"},
						},
					},
				},
			},
			want: []string{"Root=1-63f13f3b-4aed39aafbf3701390990cf1;Parent=ea243272ffa5c50b;Sampled=1"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {

			got := extractXRayTraceIDs(tt.args.ev)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("extractXRayContextIfExists() = %v, want %v", got, tt.want)
			}
		})
	}
}
