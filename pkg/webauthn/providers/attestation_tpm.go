...
type TpmAttestationProvider struct{}

func NewTpmAttestationProvider() *TpmAttestationProvider {
	return &TpmAttestationProvider{}
}

func (me *TpmAttestationProvider) ID() string {
	return "tpm"
}

var (
	ErrTPM = errors.New("ErrTPM")
)
...
func (me *TpmAttestationProvider) Handler(att types.AttestationObject, clientDataHash []byte) (hex.Hash, string, []interface{}, error) {
	...
	if !present {
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving ver value")
	}

	if ver != "2.0" {
		return nil, "", nil, errors.Wrap(ErrTPM, "WebAuthn only supports TPM 2.0 currently")
	}

	alg, present := att.AttStatement["alg"].(int64)
	if !present {
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving alg value")
	}
	...
	if !x509present {
		...
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving x5c value")
	}

	_, ecdaaKeyPresent := att.AttStatement["ecdaaKeyId"].([]byte)
	if ecdaaKeyPresent {
		return nil, "", nil, errors.Wrap(ErrTPM, "ECDAA attestation is not supported")
	}

	sigBytes, present := att.AttStatement["sig"].([]byte)
	if !present {
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving sig value")
	}

	certInfoBytes, present := att.AttStatement["certInfo"].([]byte)
	if !present {
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving certInfo value")
	}

	pubAreaBytes, present := att.AttStatement["pubArea"].([]byte)
	if !present {
		return nil, "", nil, errors.Wrap(ErrTPM, "Error retreiving pubArea value")
	}
	...
	if err != nil {
		return nil, "", nil, errors.Wrap(ErrTPM, "Unable to decode TPMT_PUBLIC in attestation statement")
	}
	...
	case webauthncose.RSAPublicKeyData:
		...
		if pubArea.RSAParameters.Modulus.Cmp(mod) != 0 ||
			...
			return nil, "", nil, errors.Wrap(ErrTPM, "Mismatch between RSAParameters in pubArea and credentialPublicKey")
		}
	default:
		return nil, "", nil, errors.Wrap(ErrTPM, "Unsupported public key type")
	}
	...
	if certInfo.Magic != 0xff544347 {
		return nil, "", nil, errors.Wrap(ErrTPM, "Magic is not set to TPM_GENERATED_VALUE")
	}
	// 2/4 Verify that type is set to TPM_ST_ATTEST_CERTIFY.
	if certInfo.Type != googletpm.TagAttestCertify {
		return nil, "", nil, errors.Wrap(ErrTPM, "Type is not set to TPM_ST_ATTEST_CERTIFY")
	}
	...
	if !bytes.Equal(certInfo.ExtraData, h.Sum(nil)) {
		return nil, "", nil, errors.Wrap(ErrTPM, "ExtraData is not set to hash of attToBeSigned")
	}
	...
	if !bytes.Equal(h.Sum(nil), certInfo.AttestedCertifyInfo.Name.Digest.Value) {
		return nil, "", nil, errors.Wrap(ErrTPM, "Hash value mismatch attested and pubArea")
	}
	...
	if x509present {
		...
		if !valid {
			return nil, "", nil, errors.Wrap(ErrTPM, "Error getting certificate from x5c cert chain")
		}

		aikCert, err := x509.ParseCertificate(aikCertBytes)
		if err != nil {
			return nil, "", nil, errors.Wrap(ErrTPM, "Error parsing certificate from ASN.1")
		}
		...
		if err != nil {
			return nil, "", nil, errors.Wrap(ErrTPM, fmt.Sprintf("Signature validation error: %+v\n", err))
		}
		// Verify that aikCert meets the requirements in §8.3.1 TPM Attestation Statement Certificate Requirements

		// 1/6 Version MUST be set to 3.
		if aikCert.Version != 3 {
			return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate version must be 3")
		}
		// 2/6 Subject field MUST be set to empty.
		if aikCert.Subject.String() != "" {
			return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate subject must be empty")
		}
		...
		if manufacturer == "" || model == "" || version == "" {
			return nil, "", nil, errors.Wrap(ErrTPM, "Invalid SAN data in AIK certificate")
		}

		if !isValidTPMManufacturer(manufacturer) {
			return nil, "", nil, errors.Wrap(ErrTPM, "Invalid TPM manufacturer")
		}
		...
		for _, ext := range aikCert.Extensions {
			if ext.Id.Equal([]int{2, 5, 29, 37}) {
				...
				if len(rest) != 0 || err != nil || !eku[0].Equal(tcgKpAIKCertificate) {
					return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate EKU missing 2.23.133.8.3")
				}
				ekuValid = true
			}
		}
		if !ekuValid {
			return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate missing EKU")
		}
		...
		for _, ext := range aikCert.Extensions {
			if ext.Id.Equal([]int{2, 5, 29, 19}) {
				if rest, err := asn1.Unmarshal(ext.Value, &constraints); err != nil {
					return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate basic constraints malformed")
				} else if len(rest) != 0 {
					return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate basic constraints contains extra data")
				}
			}
		}
		if constraints.IsCA {
			return nil, "", nil, errors.Wrap(ErrTPM, "AIK certificate basic constraints missing or CA is true")
		}
...
func forEachSAN(extension []byte, callback func(tag int, data []byte) error) error {
	...
	if err != nil {
		...
		return errors.Wrap(ErrTPM, "x509: trailing data after X.509 extension")
	}
