...
func TestRequest(t *testing.T) {
	...
	_, err := NewRequestWithContext(context.Background(), "GET", "://foo", nil)
	if err == nil {
		t.Fatalf("should error")
	}

	// Works with no request body
	_, err = NewRequestWithContext(context.Background(), "GET", "http://foo", nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	req, err := NewRequestWithContext(context.Background(), "GET", "/", body)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func testClientDo(t *testing.T, body interface{}) {
	// Create a request
	req, err := NewRequestWithContext(context.Background(), "PUT", "http://127.0.0.1:28934/v1/foo", body)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	client.RetryMax = 50
	client.RequestLogHook = func(logger *zerolog.Logger, req *http.Request, retryNumber int) {
...
func TestClient_Do_WithResponseHandler(t *testing.T) {
	...
	var checks int
	client.CheckRetry = func(_ context.Context, resp *Response, err error) (bool, error) {
		checks++
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			shouldSucceed = false
			// Create the request
			req, err := NewRequestWithContext(context.Background(), "GET", ts.URL, nil)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestClient_Do_fails(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create the request
			req, err := NewRequestWithContext(context.Background(), "POST", ts.URL, nil)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestClient_Get(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient().Get(context.Background(), ts.URL+"/foo/bar")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func testClientRequestLogHook(t *testing.T, logger *zerolog.Logger) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "GET" {
			t.Fatalf("bad method: %s", r.Method)
		}
	...
	client.RequestLogHook = func(logger *zerolog.Logger, req *http.Request, retry int) {
		retries = retry
	...
	// Make the request.
	resp, err := client.Get(context.Background(), ts.URL+testURIPath)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func TestClient_ResponseLogHook(t *testing.T) {
	t.Run("ResponseLogHook successfully called with zerolog TestLogger", func(t *testing.T) {
		l := zerolog.New(zerolog.NewTestWriter(t)).With().Timestamp().Logger()
		testClientResponseLogHook(t, l)
	})
	t.Run("ResponseLogHook successfully called with zerolog std out", func(t *testing.T) {
		l := zerolog.New(os.Stdout).With().Timestamp().Logger()
		testClientResponseLogHook(t, l)
	})
}

func testClientResponseLogHook(t *testing.T, logger zerolog.Logger) {
	...
	client.RetryWaitMax = 10 * time.Millisecond
	client.RetryMax = 15
	client.ResponseLogHook = func(logger *zerolog.Logger, resp *Response) {
		if resp.StatusCode == 200 {
...
func TestClient_RequestWithContext(t *testing.T) {
	...
	req, err := NewRequestWithContext(context.Background(), http.MethodGet, ts.URL, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestClient_Head(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient().Head(context.Background(), ts.URL+"/foo/bar")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func TestClient_PostForm(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient().PostForm(context.Background(), ts.URL+"/foo/bar", form)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func TestClient_BackoffCustom(t *testing.T) {
	...
	client := NewClient()
	client.Backoff = func(min, max time.Duration, attemptNum int, resp *Response) time.Duration {
	...
	// Make the request.
	resp, err := client.Get(context.Background(), ts.URL+"/foo/bar")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
