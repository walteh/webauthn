...
func TestParsedCredentialCreationData_Verify(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := pcc.Verify(NewNoneAttestationProvider(), tt.args.storedChallenge, hex.Hash{}, tt.args.verifyUser, tt.args.relyingPartyID, tt.args.relyingPartyOrigin); (err != nil) != tt.wantErr {
