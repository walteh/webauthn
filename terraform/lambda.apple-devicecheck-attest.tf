resource "null_resource" "apple_devicecheck_attest" {
  triggers = { src_hash = "${data.archive_file.core.output_sha}" }
  provisioner "local-exec" {
    environment = {
      dir = local.apple_devicecheck_attest_dir
      tag = "${aws_ecr_repository.core.repository_url}:${local.apple_devicecheck_attest_tag}"
    }
    command = local.lambda_docker_deploy_command
  }
}

data "aws_ecr_image" "apple_devicecheck_attest" {
  depends_on      = [null_resource.apple_devicecheck_attest]
  repository_name = aws_ecr_repository.core.name
  image_tag       = local.apple_devicecheck_attest_tag
}


resource "aws_lambda_function" "apple_devicecheck_attest" {
  depends_on = [
  ...
  image_uri        = "${aws_ecr_repository.core.repository_url}:${local.apple_devicecheck_attest_tag}"
  role             = aws_iam_role.apple_devicecheck_attest.arn
  memory_size      = 128
  timeout          = 120
  package_type     = "Image"
  publish          = true
  architectures    = ["arm64"]
  source_code_hash = trimprefix(data.aws_ecr_image.apple_devicecheck_attest.image_digest, "sha256:")
...
resource "aws_iam_role" "apple_devicecheck_attest" {
  ...
  inline_policy {
    ...
    policy = data.aws_iam_policy_document.apple_devicecheck_attest_lambda_inline.json
  }
...
data "aws_iam_policy_document" "apple_devicecheck_attest_lambda_inline" {
  /* statement {
    effect    = "Allow"
...
resource "aws_apigatewayv2_route" "apple_devicecheck_attest" {
  api_id             = aws_apigatewayv2_api.auth.id
  route_key          = "POST /apple/devicecheck/attest"
  authorization_type = "NONE"
  target             = "integrations/${aws_apigatewayv2_integration.apple_devicecheck_attest_lambda.id}"
}

resource "aws_lambda_permission" "apple_devicecheck_attest" {
  statement_id  = "${local.app_stack}-apple-devicecheck-attest-AllowExecutionFromApiGatewayRoute"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.apple_devicecheck_attest.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.auth.execution_arn}/*/*/attest"
}

resource "aws_apigatewayv2_integration" "apple_devicecheck_attest_lambda" {
  api_id                 = aws_apigatewayv2_api.auth.id
  integration_type       = "AWS_PROXY"
  integration_method     = "POST"
  integration_uri        = aws_lambda_function.apple_devicecheck_attest.invoke_arn
